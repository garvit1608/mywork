...
(function (a, b, c, d) {
    ...
    function V(e) {
        ...
        k.each(function (a) {
			...
			if(typeof this.caption != 'undefined') {
				...
				if(typeof splitCpt[1] != 'undefined') {
					this.caption = splitCpt[0]+'<h3 class="fotorama__caption_new_line">'+splitCpt[1]+'</h3>';
				}
			}
        ...
        if (f().caption) {
            var cD = c('<h1 class="fotorama__caption"></h1>');
	...
	c(a).load(function(){
        $('.chr_slider, .fotorama-nav-wrap-custom').stop(true).animate({opacity : 1}, 200);
	});
    var winScr = 0;
    ...
    function fotoramaParallax(){
        winScr = $(window).scrollTop();
        $('.chr_slider.fotorama .fotorama__frame').find('img').css({top : winScr/1.2});
    }
